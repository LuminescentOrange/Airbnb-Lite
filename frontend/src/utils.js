//后端通信
const domain = "https://staybooking-346607.uc.r.appspot.com"


export const login = (credential, asHost) => {
    const loginUrl = `${domain}/authenticate/${asHost ? "host" : "guest"}`; //template literals
    return fetch(loginUrl, { //fetch接受url和data
        method: "POST",
        headers: {
        "Content-Type": "application/json", // 转换成json
        },
        body: JSON.stringify(credential),
    }).then((response) => { //网络没问题 执行then
